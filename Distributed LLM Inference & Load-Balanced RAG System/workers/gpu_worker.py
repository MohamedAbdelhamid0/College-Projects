...
class GPUWorker:
    ...
    def is_healthy(self) -> bool:
        ...
        if self._failed:
            return False
        try:
            # Fast ping first — just check if Ollama is alive at all
            r = requests.get(
                f"{self.ollama_host}/api/tags",
                timeout=3,                          # was using check_model_available default (much slower)
    ...
    def mark_unhealthy(self) -> None:
        """Mark worker as unhealthy (alias for simulate_failure).
        
        Called by the load generator during failure simulation tests.
        """
        self.simulate_failure()

    def mark_healthy(self) -> None:
        """Mark worker as healthy again (alias for recover).
        
        Called by the load generator during failure recovery tests.
        """
        self.recover()
