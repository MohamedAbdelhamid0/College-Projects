...
from client.prompts import PROMPT_POOL, select_prompt
...
def simulate_user(
    ...
    with ctx:
        prompt = select_prompt(user_id, mode=prompt_mode, seed=prompt_seed)
        request = Request(id=user_id, query=prompt)
        try:
...
def run_load_test(
    ...
    print(f"  Prompt mode     : {prompt_mode} ({len(PROMPT_POOL)} prompts)")
    ...
    for i in range(num_users):
        t = threading.Thread(
            ...
            args=(scheduler, i, results, lock, semaphore, prompt_mode, prompt_seed),
        )
        threads.append(t)
        t.start()
